...
unless ($fusion_name && $genome_lib_dir) {
...
main: {
    ...
    my ($gene_left, $gene_right) = split("--", $fusion_name);
...
sub get_candidate_breaks {
    ...
    my $orient = $gene_obj->get_transcribed_orient();
    ...
    foreach my $exon (@exons) {
        ...
        if ($orient eq '+') {
            if ($fusion_side eq 'left' && $exon ne $last_exon) {
                $breaks{$rend}++;
            }
            elsif ($exon ne $first_exon) {
                $breaks{$lend}++;
            }
        }
        elsif ($orient eq '-') {
            if ($fusion_side eq 'left' && $exon ne $first_exon) {
                $breaks{$lend}++;
            }
            elsif ($exon ne $last_exon) {
                $breaks{$rend}++;
            }
        }
    }
